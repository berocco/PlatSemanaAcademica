(function() {
	...
	function ScheduleTemplate( element ) {
		...
		try {
			this.updateButton = this.element.getElementsByClassName('updatePop')[0];
			this.deleteButton = this.element.getElementsByClassName('deletePop')[0];
		}
		finally{
			console.log('aefaef');
		};
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		this.modalDate.setAttribute('style', 'color: #fff; margin: auto;');
		this.modalEventName.setAttribute('style', 'color: #fff; margin: auto;');
		this.modal.setAttribute('data-event', target.getAttribute('data-event'));

		if (this.updateButton!=null) {
			this.updateButton.setAttribute('onclick', "document.getElementById('upd"+target.getAttribute('pk')+"').click()");
			this.deleteButton.setAttribute('onclick', "document.getElementById('del"+target.getAttribute('pk')+"').click()");
		};
		...
		if( mq == 'mobile' ) {
			...
			self.modalHeaderBg.setAttribute('style', 'height: '+eventHeight+'px; width: '+eventWidth+'px; transform: scaleY('+HeaderBgScaleY+');   background-image: -moz-linear-gradient(90deg, #9f0000 0%, #c81919 100%); background-image: -webkit-linear-gradient(90deg, #9f0000 0%, #c81919 100%); background-image: -ms-linear-gradient(90deg, #9f0000 0%, #c81919 100%);');
	...
	ScheduleTemplate.prototype.loadEventContent = function(content) {
        ...
        ;
        httpRequest.open('GET', './detalhes/' +content);
        httpRequest.send();
    }
	;
